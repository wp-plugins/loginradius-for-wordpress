...
if ( ! class_exists('Login_Helper') ) {
    ...
    class Login_Helper {
         ...
         */
        public static function login_user($userId, $profileData, $firstLogin = false, $isRegister = false) {
            ...
            if(isset($lr_raas_custom_obj_settings['enable_custom_obj']) && $lr_raas_custom_obj_settings['enable_custom_obj']==1){
                do_action('lr_custom_obj_render_form',$userId,'');
            }else{
                self::lr_user_login($userId,$isRegister);
            }
        }

        public static function lr_user_login($userId,$isRegister=false){
        ...
        public static function register_user($profileData, $loginRadiusPopup = false) {
            ...
            if (isset($socialId) && !empty($socialId)) {
                ...
                if ( !is_wp_error( $user_id ) ) {
                    ...
                    self::login_user($user_id, $profileData, true, true);
                } else {
        ...
        public static function filter_loginradius_data_for_wordpress_use($userProfileObject) {
            if(class_exists('LR_Social_Profile_Data_Function')){
                ...
                $profileData['ID'] = isset($userProfileObject->ID) ? $userProfileObject->ID : '';
                $profileData['uid'] = isset($userProfileObject->Uid) ? $userProfileObject->Uid : '';
