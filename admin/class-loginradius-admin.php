...
if ( !class_exists( 'Login_Radius_Admin' ) ) {
    ...
    class Login_Radius_Admin {
        ...
        /**
         * Login_Radius_Admin calss instance
         *
         * @var string
         */
        ...
        public function admin_menu() {
            ...
            $page = add_menu_page( 'LoginRadius', '<b>LoginRadius</b>', 'manage_options', 'LoginRadius', 'Login_Radius_Admin::options_page', LOGINRADIUS_PLUGIN_URL . 'assets/images/favicon.ico' );
        ...
        public static function validate_options( $loginRadiusSettings ) {
            ...
            if ( empty( $loginRadiusSettings['LoginRadius_apikey'] ) || empty( $loginRadiusSettings['LoginRadius_secret'] ) ) {
                // If empty apikey or secret key, return settings
                return $loginRadiusSettings;
            } else {
                // if none of apikey and secret are empty
                $apiKey = sanitize_text_field( $loginRadiusSettings['LoginRadius_apikey'] );
                $apiSecret = sanitize_text_field( $loginRadiusSettings['LoginRadius_secret'] );
                $encodeString = Admin_Helper:: get_encoded_settings_string( $loginRadiusSettings );
                ...
                if ( self:: api_validation_response( $apiKey, $apiSecret, $encodeString ) ) {
                    ...
                    $loginRadiusSettings['LoginRadius_numColumns'] = is_numeric( $loginRadiusSettings['LoginRadius_numColumns'] ) ? $loginRadiusSettings['LoginRadius_numColumns'] : '';
                    return $loginRadiusSettings;
                } else {
                    // Api or Secret is not validat or something wrong happened while getting response from LoginRadius api
                    ...
                    $loginRadiusSettingsold = get_option( 'LoginRadius_settings' );
                    add_settings_error( 'LoginRadius_settings', esc_attr( 'settings_updated' ), $message, 'error' );
                    add_action( 'admin_notices', array('Admin_Helper', 'display_admin_settings_errors') );
